...
export const CreateAtividade = (props: CreateAtividadeProps) => {
  ...
  const filteredTitulo = FaseData.filter(
    (faseData) =>
      ...
      <S.ModalArea >
        <S.ModalContent id="modal-content">
          ...
          <S.ChooseUser>
              ...
              <input type="text" onChange={handleNewAtividade} placeholder="Adicionar Outro" style={{color: '#fff',fontFamily: 'Rubik', border: 'none',borderRadius: '5px', padding: '0 2em',backgroundColor: '#5C6170', height: '44px', width: '100%'}}
              />
              <button onClick={() => {if(newAtividade.length > 0){setListNewAtividades((old) => [...old, {url: 'new', titulo: newAtividade, id: uuidv4()}])}}} style={{position: 'absolute', right: '0', translate:'-35% 30%', top: '0', border: 'none', backgroundColor: 'transparent'}}>
                ...
                {listNewAtividades.map((categ: any) => {
                  return(
                    ...
                    >
                      ...
                      <button onClick={() => {if(newAtividade.length > 0){setListNewAtividades(listNewAtividades.filter((atv) => atv.id != categ.id))}}} style={{position: 'absolute', right: '0', translate:'-35% 65%', top: '0', border: 'none', backgroundColor: 'transparent'}}>
